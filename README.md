...
#### 1. 捐赠登记 (`/donate`)
- 访问 http://127.0.0.1:5000/donate
- 填写捐赠者姓名（可选，支持匿名）
- 输入联系电话
- 选择物品类别（衣物、图书、生活用品、综合捐赠）
- 选择新旧程度
...
- 可点击溯源码直接跳转到该物品的详细溯源页面

#### 4. 签收确认 (`/receive`)
- 访问 http://127.0.0.1:5000/receive
- 输入溯源码查询物品
- 查看物品照片、状态、捐赠者信息
...
**系统默认仅监听 `127.0.0.1:5000`**，只有本机可访问。

要让局域网内其他设备访问，需修改启动配置：
